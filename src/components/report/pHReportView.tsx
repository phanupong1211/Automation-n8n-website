...
export default function pHReportView({ data }: { data: pHReportData }) {
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <a href={`/dashboard/asset/pH/${encodeURIComponent(data.tag)}`} className="text-sm text-blue-600 hover:underline">← กลับ</a>
          <h1 className="mt-1 text-2xl font-semibold text-slate-900">
            pH Sensor Report <span className="text-slate-700">#{data.reportNo}</span>
          </h1>
